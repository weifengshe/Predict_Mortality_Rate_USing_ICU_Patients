...
// svm model
//train svm model on train 
//convert data to sparsevector

//structured data for Train
val trainStructureLabeled = trainAllData.map{line =>  
         LabeledPoint(line(8).toString.toDouble, 
            Vectors.dense(line(3).toString.toDouble,  line(4).toString.toDouble,
            line(5).toString.toDouble,  line(6).toString.toDouble,
            line(7).toString.toDouble) )}

trainStructureLabeled.cache()

//structured data for Test
val testStructureLabeled = testAllData.map{line =>  
             LabeledPoint(line(8).toString.toDouble, 
             Vectors.dense(line(3).toString.toDouble,  line(4).toString.toDouble,
             line(5).toString.toDouble,  line(6).toString.toDouble,
             line(7).toString.toDouble) )}

testStructureLabeled.cache()
//notes data for train

val temp = trainAllData.map{line =>  
    val vect = line(0).faltMap( _.toString.toDouble)
    (vect)
}

val trainNotesLabeled = trainAllData.map{line =>  
    //val vect = line(0).map(r => r.toString.toDouble)
    val features = line.getAs[Vector]("TopicVector")
         LabeledPoint(line(8).toString.toDouble, features)
     }

trainNotesLabeled.cache()

//notes data for Test
val testNotesLabeled = testAllData.map{line =>  
             val features = line.getAs[Vector]("TopicVector")
             LabeledPoint(line(8).toString.toDouble, features)}

testNotesLabeled.cache()

//all data for Train
val trainAllDataLabeled = trainAllData.map{line =>  
        val features = line.getAs[Vector]("TopicVector")
         LabeledPoint(line(8).toString.toDouble, 
            Vectors.dense(Array(line(3).toString.toDouble,  line(4).toString.toDouble,
            line(5).toString.toDouble,  line(6).toString.toDouble,
            line(7).toString.toDouble) ++ features.toArray)) }

trainAllDataLabeled.cache()

//all data for Test
val testAllDataLabeled = testAllData.map{line =>  
         val features = line.getAs[Vector]("TopicVector")
         LabeledPoint(line(8).toString.toDouble, 
            Vectors.dense(Array(line(3).toString.toDouble,  line(4).toString.toDouble,
            line(5).toString.toDouble,  line(6).toString.toDouble,
...
testAllDataLabeled.cache()


def cal_svm_auc(trainLabeled: org.apache.spark.rdd.RDD[org.apache.spark.mllib.regression.LabeledPoint], 
    testLabeled: org.apache.spark.rdd.RDD[org.apache.spark.mllib.regression.LabeledPoint]){
var numIterations = 0
var bestauROC = -1000.00

for(numIterations <- 50 to 100){

    val model = SVMWithSGD.train(trainLabeled, numIterations)

    //model.clearThrehold()

    val scoreAndLabels = testLabeled.map { point =>
...
println("Best roc: " + bestauROC)
(bestauROC)
}

val auc_svm_structured = cal_svm_auc(trainStructureLabeled, testStructureLabeled)

println("auc for test dataset of only structured data on svm model: " + auc_svm_structured)

val auc_svm_notes = cal_svm_auc(trainNotesLabeled, testNotesLabeled)

println("auc for test dataset of only notes data on svm model: " + auc_svm_notes)

val auc_svm_AllData = cal_svm_auc(trainAllDataLabeled, testAllDataLabeled)

println("auc for test dataset of only notes data on svm model: " + auc_svm_AllData)
